...
```text
cydia/debs/cydia_1.1.26_iphoneos-arm64.deb
```
...
Keep the metadata in `cydia/cydia.control` when generating the repository index. Upload the [repository icon](RepositoryAssets/README.md) so it is visible before installation. The version is **1.1.26**, so an installed 1.1.25 is eligible for a normal package update after the repository index is refreshed.
