...
usage() {
    printf '%s\n' \
        ...
        '' \
        'Builds and verifies clean Cydia 1.1.30 on a rootless iOS 15+ device.' \
...
log="${TMPDIR:-/tmp}/Cydia_1.1.30_device-build.log"
: > "$log"

set +e
{
    printf 'Cydia 1.1.30 Rootless clean device build\n'
    printf 'source: %s\n' "$root"
    printf 'SDKROOT: %s\n' "$SDKROOT"
    printf 'jobs: %s\n\n' "$jobs"
