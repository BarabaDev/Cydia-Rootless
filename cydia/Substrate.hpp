...
static inline Type_ *MSHookIvarPointer(id self, const char *name) {
    if (self == nil || name == NULL)
        return NULL;
    Ivar ivar(class_getInstanceVariable(object_getClass(self), name));
    if (ivar == NULL)
        return NULL;
    return reinterpret_cast<Type_ *>(reinterpret_cast<char *>(self) + ivar_getOffset(ivar));
}
