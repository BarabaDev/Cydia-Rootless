...
static pkgCache::VerIterator CYRootlessCandidateVersion(pkgCacheFile &cache, pkgCache::PkgIterator package) {
    ...
    for (pkgCache::VerIterator version(package.VersionList()); !version.end(); ++version) {
        ...
        if (best.end() || priority > bestPriority ||
            (priority == bestPriority && version.CompareVer(best) > 0)) {
            best = version;
    ...
    while (std::getline(is, line)) {
        ...
        if (finish_r(data, size)) {
            ...
            if ([finish isEqualToString:@"uicache"]) {
                ...
                int index = [Finishes_ indexOfObject:requested];
...
- (bool) popErrorWithTitle:(NSString *)title {
    ...
    while (!_error->empty()) {
        ...
        if (warning && no_pubkey(error.c_str())) {
            CYRootlessDiag(@"APT", @"level=INFO task=%@ suppressedFromUI=1 reason=NO_PUBKEY", title ?: @"<nil>");
            continue;
        }

        if (warning && IsKnownBigBossLegacySHA1Warning(error)) {
            CYRootlessDiag(@"APT", @"level=INFO task=%@ suppressedFromUI=1 reason=BigBoss-legacy-SHA1", title ?: @"<nil>");
            lprintf("W:[BigBoss legacy SHA1 signature verified; warning hidden from Refresh UI]\n");
            continue;
        }
  ...
  open:
    ...
    if (!opened) {
        ...
        while (!_error->empty()) {
            ...
            if (warning && IsKnownBigBossLegacySHA1Warning(error)) {
                lprintf("W:[BigBoss legacy SHA1 signature verified; warning hidden from cache UI]\n");
                continue;
            }
...
- (void) perform {
    ...
    for (pkgAcquire::ItemIterator item = fetcher_->ItemsBegin(); item != fetcher_->ItemsEnd(); item++) {
        ...
        NSString *diagnosticURI([NSString stringWithUTF8String:uri.c_str()]);
        NSString *diagnosticError([NSString stringWithUTF8String:error.c_str()]);
        CYRootlessDiag(@"DOWNLOAD", @"level=ERROR uri=%@ message=%@",
            CYRootlessDiagnosticsText(diagnosticURI ?: @"<non-UTF8 URI>"),
            CYRootlessDiagnosticsText(diagnosticError ?: @"<non-UTF8 error>"));
...
- (void) updateWithStatus:(CancelStatus &)status {
    ...
    for (pkgSourceList::const_iterator source = list.begin(); source != list.end(); ++source)
        ++sourceCount;
    CYRootlessDiag(@"REFRESH", @"sourceEntries=%zu", sourceCount);
    ...
    if (cancelled)
        ...
        fatal = [self popErrorWithTitle:title forOperation:success];
    ...
    [delegate_ performSelectorOnMainThread:@selector(releaseNetworkActivityIndicator) withObject:nil waitUntilDone:YES];
    CYRootlessDiag(@"REFRESH", @"end status=%@ listUpdateSuccess=%d fatal=%d cancelled=%d quarantined=%zu durationMs=%llu",
        cancelled ? @"cancelled" : ((success && !fatal) ? @"ok" : @"failed"),
        success, fatal, cancelled, quarantined,
        (unsigned long long) ((_timestamp - diagnosticsStart) / 1000));
}
...
- (void) dealloc {
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(flushProgressUpdate) object:nil];
...
- (id) initWithDatabase:(Database *)database delegate:(id)delegate {
    if ((self = [super init]) != nil) {
        ...
        ready_ = false;
        progressUpdateScheduled_ = false;
...
// dpkg can emit many status/output lines back-to-back.  The old
// synchronous JavaScript dispatch path could keep the main run loop busy
// processing CydiaProgressUpdate events without giving WebKit a display turn,
// so install/remove/upgrade output often appeared all at once only after dpkg
// finished.  Source Refresh did not expose the problem because network latency
// naturally spaces its events out.  Coalesce rapid progress changes onto a
// short main-run-loop timer: the transaction thread never waits on WebKit, and
// WebKit gets regular opportunities to paint the same original progress page.
...
- (void) updateProgress {
    ...
    if (progressUpdateScheduled_)
        return;

    progressUpdateScheduled_ = true;
    // Schedule the shared Sources/transaction progress repaint in
    // NSRunLoopCommonModes.  This keeps the same original progress page live
    // across modal transitions and other run-loop modes instead of letting a
    // fast dpkg transaction starve the repaint until completion.
    [self performSelector:@selector(flushProgressUpdate)
               withObject:nil
               afterDelay:0.04
    ...
    switch (Finish_) {
        ...
        case 4:
            ...
            if (RebootMode_ == 1) {
                ...
                // hardware/device reboot. Keep the visible button text
                // "Reboot Device" for compatibility, but execute launchctl
                // reboot userspace as root through cydo.
...
- (void) invoke:(NSInvocation *)invocation withTitle:(NSString *)title {
    ...
    [self setTitle:title];
    // Make the original progress WebView visible before the
    // transaction thread starts. Otherwise fast dpkg operations can finish
    // while the local progress page is still loading, causing all lines to
    // appear at once only on the final screen.
    [self view];
    NSDate *deadline([NSDate dateWithTimeIntervalSinceNow:1.5]);
    while (!ready_ && [deadline timeIntervalSinceNow] > 0)
    ...
    // Do not leave the final state behind a throttled update.  Flush it
    // immediately so the finish action and last dpkg lines are visible as soon
    // as the transaction completes.
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(flushProgressUpdate) object:nil];
    progressUpdateScheduled_ = false;
    [self flushProgressUpdate];

    CYRootlessDiag(@"FINISH", @"transaction title=%@ finishIndex=%d finishLabel=%@ durationMs=%llu",
        title, Finish_, [progress_ finish],
        (unsigned long long) ((_timestamp - diagnosticsStart) / 1000));
...
- (void) addProgressEvent:(CydiaProgressEvent *)event {
    ...
    NSString *message(CYRootlessDiagnosticsText([event message]));
...
- (void) drawContentRect:(CGRect)rect {
    ...
    if (icon_ != nil) {
        if (source_ != nil) {
            // Repository artwork is supplied in many different pixel
            // sizes.  Draw every source inside the same 32x32 point container
            // and apply one consistent rounded mask, while preserving aspect
            // ratio with an aspect-fill crop.  This changes presentation only;
            // the downloaded repository image itself is never modified.
            CGRect target = Retina(CGRectMake(10.0f, 10.0f, 32.0f, 32.0f));
            CGSize size = [(UIImage *) icon_ size];

            if (size.width > 0.0f && size.height > 0.0f) {
                CGFloat scale = MAX(target.size.width / size.width, target.size.height / size.height);
                CGSize drawn = CGSizeMake(size.width * scale, size.height * scale);
                CGRect image = CGRectMake(
                    target.origin.x + (target.size.width - drawn.width) / 2.0f,
                    target.origin.y + (target.size.height - drawn.height) / 2.0f,
                    drawn.width,
                    drawn.height
                );
                ...
                [[UIBezierPath bezierPathWithRoundedRect:target cornerRadius:7.0f] addClip];
                [icon_ drawInRect:Retina(image)];
                CGContextRestoreGState(context);
            }
        } else {
            // Keep the original All Sources folder artwork behavior.
            CGRect rect;
            rect.size = [(UIImage *) icon_ size];
...
> {
    ...
    NSURLConnection *trivial_zst_;
    NSURLConnection *trivial_plain_;
...
- (void) dealloc {
    ...
    [self _releaseConnection:trivial_zst_];
    [self _releaseConnection:trivial_plain_];
...
- (void) _endConnection:(NSURLConnection *)connection {
    ...
    if (connection == trivial_bz2_)
        ...
        field = &trivial_xz_;
    else if (connection == trivial_zst_)
        field = &trivial_zst_;
    else if (connection == trivial_plain_)
        field = &trivial_plain_;
    _assert(field != NULL);
    [connection release];
    *field = nil;
    ...
    if ([context isEqualToString:@"source"]) {
        switch (button) {
            case 1: {
                ...
                // Preserve the original lightweight repository check,
                // but recognize formats used by current rootless repositories too.
                // A source is accepted if any one Packages representation exists.
                ...
                trivial_zst_ = [[self _requestHRef:[href_ stringByAppendingString:@"Packages.zst"] method:@"HEAD"] retain];
                trivial_plain_ = [[self _requestHRef:[href_ stringByAppendingString:@"Packages"] method:@"HEAD"] retain];
...
- (ProgressController *) invokeNewProgress:(NSInvocation *)invocation forController:(UINavigationController *)navigation withTitle:(NSString *)title {
    ...
    if (navigation != nil) {
        // Confirm -> Progress is a navigation push. Sources uses its own
        // modal progress controller, but both now use the same screen-first
        // ordering before work begins. Let this
        // button event return and let UIKit commit the push before APT/dpkg
        // starts.  This makes the original progress page visible first, so
        // Downloading/Running dpkg/Preparing/Unpacking/Configuring/Removing
        // events can be painted live instead of appearing all at once at end.
...
int main(int argc, char *argv[]) {
    ...
    int legacySourceCleanup(system("/var/jb/usr/libexec/cydia/cydo --cleanup-legacy-source-link"));
    if (legacySourceCleanup != 0)
        CYRootlessDiag(@"SOURCES", @"level=ERROR legacy symlink cleanup rawStatus=%d", legacySourceCleanup);
    ...
    const char *methodsPath = systemMethods != NULL ? systemMethods : "/var/jb/Applications/Cydia.app";
    ...
    // Rootless iOS 15+: BigBoss still signs its legacy Cydia
    // Release metadata with SHA1 using a 1024-bit DSA key.  Current APT
    // classifies both as untrusted and rejects the repository.  Preserve
    // signature verification, but downgrade SHA1 to WEAK so verification can
    // succeed, and explicitly permit the legacy dsa1024 signing algorithm.
    // Filter only the exact known BigBoss weak-SHA1 notice from the
    // Refresh UI after it has already been recorded in Cydia's log.
    //
    // This is intentionally NOT Acquire::AllowInsecureRepositories and does
    // not mark any source as trusted=yes: the Release signature must still
    // validate against a key already present in APT's trusted keyrings.
    _config->Set("APT::Hashes::SHA1::Weak", true);
    _config->Set("APT::Key::Assert-Pubkey-Algo",
        ">=rsa2048,ed25519,ed448,nistp256,nistp384,nistp512,brainpoolP256r1,brainpoolP320r1,brainpoolP384r1,brainpoolP512r1,secp256k1,dsa1024");
