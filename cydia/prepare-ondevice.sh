...
missing=0
for tool in clang clang++ make ldid dpkg-deb ar git perl sed find install_name_tool tar; do
...
DPKG_DEB_HELP="$(dpkg-deb --help 2>&1 || true)"
if ! grep -Fq -- '--root-owner-group' <<<"$DPKG_DEB_HELP"; then
    echo
    echo "[miss] dpkg-deb does not support --root-owner-group"
    echo "Upgrade the Procursus dpkg package, then run the build again."
    exit 10
fi
echo "[ok]   dpkg-deb supports --root-owner-group (fakeroot is not required)"
...
if [[ ! -f /var/jb/usr/include/gnutls/gnutls.h || ! -e /var/jb/usr/lib/libgnutls.dylib ]]; then
    ...
    echo "Install it, then run the root device-build.sh command again:"
...
is_cydia_apt_tree() {
    ...
    [[ -f "$src/apt-pkg/tagfile-order.c" ]] || return 1
...
if ! is_pinned_import; then
    ...
    if [[ $imported -eq 0 ]]; then
        ...
        echo "  APT64_SOURCE=/full/path/to/apt-bingner ../device-build.sh"
        exit 11
    fi
fi
...
# tagfile-order.c is not a separately compiled translation unit. The pinned
# tagfile.cc includes it textually, so a source-pruning pass must retain it even
# though its .c suffix does not appear in the makefile object list.
grep -Fq '#include "tagfile-order.c"' apt64/apt-pkg/tagfile.cc || {
    echo "[apt64] pinned tagfile.cc no longer includes tagfile-order.c as expected"
    exit 11
}
[[ -f apt64/apt-pkg/tagfile-order.c && ! -L apt64/apt-pkg/tagfile-order.c ]] || {
    echo "[apt64] required textual source fragment is missing: apt-pkg/tagfile-order.c"
    exit 11
}
echo "[ok]   pinned APT textual source fragment tagfile-order.c"
...
# Flatten the pinned contrib/deb headers into apt64/apt-pkg for the build.
# Every Cydia/APT include then resolves through the canonical -Iapt64 path;
# no extractor, filesystem or compiler behavior can hide an overlay symlink.
# Implementation files remain in their canonical pinned source directories.
rm -rf apt64-contrib apt64-deb
cp -p apt64/apt-pkg/contrib/*.h apt64/apt-pkg/
cp -p apt64/apt-pkg/deb/*.h apt64/apt-pkg/

[[ -f apt64/apt-pkg/hashes.h && ! -L apt64/apt-pkg/hashes.h ]] || {
    echo "[apt64] failed to expose hashes.h in the canonical include tree"
    exit 11
}
[[ -f apt64/apt-pkg/debindexfile.h && ! -L apt64/apt-pkg/debindexfile.h ]] || {
    echo "[apt64] failed to expose debindexfile.h in the canonical include tree"
    exit 11
}
...
if SDK="$(./ondevice-sdk.sh)"; then
    ...
    echo "then rerun ../device-build.sh"
    exit 12
fi

if ! printf '#include <apt-pkg/hashes.h>\n' | \
    /var/jb/usr/bin/clang++ -E -x c++ -isysroot "$SDK" \
        -Iapt64 -Iapt64/methods -Iapt-extra -ICompat - >/dev/null; then
    echo
    echo "Pinned APT header preflight failed: clang cannot resolve apt-pkg/hashes.h."
    exit 13
fi
echo "[ok]   clang resolves apt-pkg/hashes.h through -Iapt64"
