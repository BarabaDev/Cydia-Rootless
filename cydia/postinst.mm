...
static bool FixApplications() {
    ...
    char destiny[PATH_MAX];
    int destinyLength(snprintf(destiny, sizeof(destiny), "%s%s", temp, APPLICATIONS));
    if (destinyLength < 0 || static_cast<size_t>(destinyLength) >= sizeof(destiny))
        goto undo;
...
int main(int argc, const char *argv[]) {
    ...
    // Rootless public build: preserve the bootstrap's existing APT sources.
    // Cydia and Sileo share /var/jb/etc/apt; no hardcoded cydia.list is installed here.
    // Keep removing only Cydia's obsolete legacy repository pin file from
    // earlier development revisions so candidate selection follows the bootstrap's
    // current APT policy. User/source configuration is not touched.
    if (unlink("/var/jb/etc/apt/preferences.d/cydia") == -1 && errno != ENOENT)
        perror("unlink(/var/jb/etc/apt/preferences.d/cydia)");
