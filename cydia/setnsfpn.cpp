...
#define _syscall(expr) ({ decltype(expr) _value = static_cast<decltype(expr)>(-1); for (;;) { \
    _value = (expr); \
    if ((long) _value != -1) \
        break; \
    if (errno != EINTR) { \
...
static int setnsfpn(const char *path, size_t before, Recurse recurse) {
    int rawfd;
    do {
        rawfd = open_dprotected_np(path, O_RDONLY | O_SYMLINK, 0, O_DP_GETRAWENCRYPTED);
    } while (rawfd == -1 && errno == EINTR);
    if (rawfd == -1) {
        perror("open_dprotected_np");
        return -1;
    }
    File fd(rawfd);
        ...
        for (long address(0);;) {
            ...
            while (next != stop) {
                ...
                if (dir->d_ino == 0);
                    ...
                    std::vector<char> sub(both + 1, '\0');
                    memcpy(sub.data(), path, before);
                    sub[before] = '/';
                    memcpy(sub.data() + before + 1, name, after);
                    sub[both] = '\0';
                    ...
                    switch (dir->d_type) {
                        ...
                        case DT_DIR:
                            if (setnsfpn(sub.data(), both, RecurseYes) != 0)
                                return -1;
                            break;
                        case DT_REG:
                            if (setnsfpn(sub.data(), both, RecurseNo) != 0)
                                return -1;
                            break;
                    }
                }
...
int main(int argc, const char *argv[]) {
    if (argc != 2) {
        fprintf(stderr, "usage: setnsfpn PATH\n");
        return 2;
    }
