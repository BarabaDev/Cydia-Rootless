...
int HttpMethod::Loop()
{
   ...
   {      
	 ...
	 continue;
      ...
      // The embedded CFNetwork transport follows redirects itself so APT can
      // continue to verify the archive against the original Packages entry.
      // Keep an explicit, bounded redirect chain: modern paid repositories
      // commonly answer with 307/308 and signed object-storage URLs.
      unsigned int redirectCount = 0;
      std::set<std::string> redirectHistory;
      char *url = strdup(uriString.c_str());
    url:
      URI uri = std::string(url);
      std::string hs = uri.Host;

      if (!redirectHistory.insert(static_cast<std::string>(uri)).second) {
         _error->Error("Redirection loop");
         Fail();
         free(url);
         continue;
      ...
      // FIX08 R5: current repositories such as Procursus reject the obsolete
      // Telesphoreo APT identity for package archives.  Keep the neutral Cydia
      ...
      CFHTTPMessageSetHeaderFieldValue(hm, CFSTR("User-Agent"), CFSTR("Cydia/1.1.22"));
      ...
      if (Firmware_ != NULL)
         CFHTTPMessageSetHeaderFieldValue(hm, CFSTR("Sec-CH-UA-Platform-Version"), Firmware_);
      CFHTTPMessageSetHeaderFieldValue(hm, CFSTR("Sec-CH-UA-Arch"), CFSTR("\"arm\""));
      CFHTTPMessageSetHeaderFieldValue(hm, CFSTR("Sec-CH-UA-Bitness"), CFSTR("\"64\""));
      sr = CFStringCreateWithCString(kCFAllocatorDefault, Machine_, se);
      CFHTTPMessageSetHeaderFieldValue(hm, CFSTR("Sec-CH-UA-Model"), sr);
      CFRelease(sr);
...
#define _kCFStreamPropertySocketImmediateBufferTimeOut CFSTR("_kCFStreamPropertySocketImmediateBufferTimeOut")
      ...
      if (sc == 301 || sc == 302 || sc == 303 || sc == 307 || sc == 308) {
         ...
         if (sr == NULL) {
            ...
            if (++redirectCount > 10) {
               CFRelease(sr);
               _error->Error("Too many redirects");
               Fail();
               goto done_;
            }

            CFStringRef current(CFStringCreateWithCString(kCFAllocatorDefault, urs.c_str(), se));
            CFURLRef base(CFURLCreateWithString(kCFAllocatorDefault, current, NULL));
            CFURLRef target(CFURLCreateWithString(kCFAllocatorDefault, sr, base));
            CFRelease(sr);
            ...
            if (target == NULL) {
               if (base != NULL)
                  CFRelease(base);
               CFRelease(current);
               _error->Error("Invalid redirect URL");
               Fail();
               goto done_;
            }

            CFStringRef absolute(CFURLGetString(target));
            CFIndex maximum(CFStringGetMaximumSizeForEncoding(CFStringGetLength(absolute), se) + 1);
            char *next(static_cast<char *>(malloc(maximum)));
            bool converted(CFStringGetCString(absolute, next, maximum, se));
            CFRelease(target);
            if (base != NULL)
               CFRelease(base);
            CFRelease(current);
            ...
            if (!converted) {
               free(next);
               _error->Error("Invalid redirect URL encoding");
               Fail();
               goto done_;
            }

            URI redirected = std::string(next);
            bool sameScheme(redirected.Access == uri.Access);
            bool upgraded(uri.Access == "http" && redirected.Access == "https");
            if (redirected.Host.empty() || (!sameScheme && !upgraded)) {
               free(next);
               _error->Error("Unsafe redirect blocked");
               Fail();
               goto done_;
            }
