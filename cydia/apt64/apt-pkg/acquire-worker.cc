...
bool pkgAcquire::Worker::Start()
{
   ...
   // Modern iOS does not permit a multithreaded UIKit process to enter the
   // legacy fork child safely. Launch the APT method atomically instead; this
   // also prevents injected jailbreak libraries from running atfork hooks in
   // Cydia's partially initialized cold-launch process.
   posix_spawn_file_actions_t actions;
   posix_spawnattr_t attributes;
   int SpawnError = posix_spawn_file_actions_init(&actions);
   bool const ActionsReady = SpawnError == 0;
   if (SpawnError == 0)
      SpawnError = posix_spawnattr_init(&attributes);
   bool const AttributesReady = SpawnError == 0;
   if (SpawnError == 0)
      SpawnError = posix_spawn_file_actions_adddup2(&actions, Pipes[1], STDOUT_FILENO);
   if (SpawnError == 0)
      SpawnError = posix_spawn_file_actions_adddup2(&actions, Pipes[2], STDIN_FILENO);
   for (int I = 0; SpawnError == 0 && I != 4; ++I)
      SpawnError = posix_spawn_file_actions_addclose(&actions, Pipes[I]);
...
#endif
   if (SpawnError == 0)
      SpawnError = posix_spawnattr_setflags(&attributes, SpawnFlags);

   char * const Args[] = { const_cast<char *>(Calling.c_str()), nullptr };
   if (SpawnError == 0)
      SpawnError = posix_spawn(&Process, Method.c_str(), &actions, &attributes, Args, environ);
   ...
   if (SpawnError != 0)
   {
      ...
      for (int I = 0; I != 4; ++I)
         close(Pipes[I]);
      errno = SpawnError;
      return _error->Errno("posix_spawn", "Failed to start method %s", Calling.c_str());
   }
