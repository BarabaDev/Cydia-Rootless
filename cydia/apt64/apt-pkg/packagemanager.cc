				    ...
				    pkgRecords *Recs)
{
   if (Cache.BrokenCount() != 0)
      return _error->Error("Package requirements must be resolved before preparing downloads.");
   ...
   for (pkgOrderList::iterator I = List->begin(); I != List->end(); ++I)
   {
	 ...
	 continue;
      ...
      auto const version(Cache[Pkg].InstVerIter(Cache));
      if (!CYArchiveIdentity::IsUsable(version))
         return _error->Error("No usable install version is selected. Refresh Sources and review the package requirements.");
      new pkgAcqArchive(Owner,Sources,Recs,version,FileNames[Pkg->ID]);
   }
