...
public:
   pkgDPkgPMPrivate() : stdin_is_dev_null(false), status_fd_reached_end_of_file(false),
			...
			deferred_already_configured(false),
			current_status_had_fatal_error(false), progress(NULL),
   ...
   FILE *history_out;
   string dpkg_error;
   // dpkg can occasionally report an explicit configure request as an error
   // even though its live database already records that one package as fully
   // installed.  Keep that one narrowly-defined status line deferred until
   // the child exit status is known so a large transaction can continue
   // without hiding any other dpkg or maintainer-script failure.
   string single_configure_package;
   string single_configure_fullname;
   string deferred_status_error_package;
   string deferred_status_error_message;
   bool deferred_already_configured;
   bool current_status_had_fatal_error;
...
namespace
{
  ...
  static bool IsAlreadyConfiguredStatusError(std::string const &message,
         std::string const &package)
  {
    static std::string const prefix("package ");
    static std::string const suffix(" is already installed and configured");
    if (package.empty() || message.compare(0, prefix.size(), prefix) != 0 ||
        message.size() <= prefix.size() + suffix.size() ||
        message.compare(message.size() - suffix.size(), suffix.size(), suffix) != 0)
      return false;

    std::string reported(message.substr(prefix.size(),
        message.size() - prefix.size() - suffix.size()));
    std::string scheduled(package);
    std::string::size_type const reportedArch(reported.find(':'));
    std::string::size_type const scheduledArch(scheduled.find(':'));
    if (reportedArch != std::string::npos)
      reported.resize(reportedArch);
    if (scheduledArch != std::string::npos)
      scheduled.resize(scheduledArch);
    return reported == scheduled;
  }
}
   ...
   */
   ...
   else if (prefix == "status")
   {
	 ...
	 */
      if(action == "error")
      {
         // A single explicit `dpkg --configure package` can race with dpkg's
         // own trigger processing and find that exact package already fully
         // configured.  Defer only that exact idempotent result.  The child
         // exit handler below accepts it only for exit code 1 and only when
         // this invocation contained one package; every other error follows
         // the normal fatal path unchanged.
         if (IsAlreadyConfiguredStatusError(list[3], d->single_configure_package))
         {
            d->deferred_status_error_package = pkgname;
            d->deferred_status_error_message = list[3];
            d->deferred_already_configured = true;
            return;
         }
         d->current_status_had_fatal_error = true;
...
#define ADDARGC(X) Args.push_back(X); Size += sizeof(X) - 1
      ...
      size_t explicitPackageArguments = 0;
      std::string onlyPackageName;
      std::string onlyPackageFullName;
	 ...
	 for (;I != J && Size < MaxArgBytes; ++I)
	 {
					   ...
					   strcmp(I->Pkg.Arch(), "none") == 0))
	    {
	       ...
	       ADDARG(name);
	       ++explicitPackageArguments;
	       onlyPackageName = I->Pkg.Name();
	       onlyPackageFullName = I->Pkg.FullName();
	    }
	    else
	    {
...
#undef ADDARG
      ...
      d->single_configure_package.clear();
      d->single_configure_fullname.clear();
      d->deferred_status_error_package.clear();
      d->deferred_status_error_message.clear();
      d->deferred_already_configured = false;
      d->current_status_had_fatal_error = false;
      if (Op == Item::Configure && explicitPackageArguments == 1)
      {
	 d->single_configure_package = onlyPackageName;
	 d->single_configure_fullname = onlyPackageFullName;
      }
	 ...
	 break;
      ...
      {
	 // This is not a generic ignore-errors path.  Recover only when dpkg
	 // itself has said that the sole package in an explicit configure call
	 // is already installed and configured.  That state is the requested
	 // postcondition, so stopping the rest of a large queue would be wrong.
	 bool const alreadyConfigured =
	    WIFEXITED(Status) != 0 && WEXITSTATUS(Status) == 1 &&
	    d->deferred_already_configured &&
	    d->current_status_had_fatal_error == false &&
	    d->single_configure_package.empty() == false &&
	    d->single_configure_fullname.empty() == false;
	 auto const packageOps = PackageOps.find(d->single_configure_fullname);
	 auto const packageDone = PackageOpsDone.find(d->single_configure_fullname);
	 if (alreadyConfigured && packageOps != PackageOps.end() &&
	       packageDone != PackageOpsDone.end())
	 {
	    auto &done = packageDone->second;
	    if (done < packageOps->second.size())
	    {
	       PackagesDone += packageOps->second.size() - done;
	       done = packageOps->second.size();
	    }

	    std::string displayName(d->single_configure_package);
	    std::string::size_type const archSeparator(
	       d->single_configure_fullname.find(':'));
	    if (archSeparator != std::string::npos)
	       displayName.append(" (").append(
	          d->single_configure_fullname.substr(archSeparator + 1)).append(")");
	    std::string message;
	    strprintf(message, _("Installed %s"), displayName.c_str());
	    d->progress->StatusChanged(d->single_configure_fullname,
	       PackagesDone, PackagesTotal, message);
	    if (d->term_out != NULL)
	       fprintf(d->term_out,
	          "Cydia recovered idempotent configure for %s: %s\n",
	          d->single_configure_fullname.c_str(),
	          d->deferred_status_error_message.c_str());
	    std::clog << "Cydia: continuing after dpkg confirmed "
	       << d->single_configure_fullname << " is already configured" << std::endl;
	    continue;
	 }

	 // If the deferred line did not meet every recovery condition, publish
	 // it now and preserve the ordinary fatal behavior.
	 if (d->deferred_status_error_message.empty() == false)
	 {
	    d->progress->Error(d->deferred_status_error_package,
	       PackagesDone, PackagesTotal, d->deferred_status_error_message);
	    ++pkgFailures;
	    WriteApportReport(d->deferred_status_error_package.c_str(),
	       d->deferred_status_error_message.c_str());
	 }
