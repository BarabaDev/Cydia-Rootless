...
// Include Files							/*{{{*/
#include <config.h>
#include "CyteKit/ArchiveIdentity.hpp"
...
bool pkgAcqMetaBase::CheckStopAuthentication(pkgAcquire::Item * const I, const std::string &Message)
{
   ...
   // Modern rootless repositories are sometimes still signed by a legacy key
   // which the bootstrap intentionally does not ship. If the application has
   // explicitly enabled unsigned-repository compatibility, do not let an old
   // cached Release file short-circuit the normal insecure-repository path for
   // NO_PUBKEY alone. The caller will parse the fresh Release and continue to
   // verify Packages indexes against its hashes. Bad signatures, NODATA and
   // every other authentication failure retain APT's strict old-index path.
   bool const MissingPublicKey = GPGError.find("NO_PUBKEY") != string::npos;
   bool const AllowMissingPublicKey = MissingPublicKey &&
      TargetIsAllowedToBe(TransactionManager->Target, InsecureType::UNSIGNED);
   if (FileExists(Final) && AllowMissingPublicKey == false)
   {
      ...
      return true;
   } else if (GPGError.find("NODATA") != string::npos) {
...
pkgAcqArchive::pkgAcqArchive(pkgAcquire *const Owner, pkgSourceList *const Sources,
			     ...
			     string &StoreFilename) : Item(Owner), d(NULL), LocalSource(false), Version(Version), Sources(Sources), Recs(Recs),
						      ...
						      Trusted(false)
{
   if (!CYArchiveIdentity::IsUsable(Version))
   {
      Status = StatError;
      ErrorText = _("The selected package metadata is incomplete. Refresh Sources and try again.");
