...
grep -A1 '<key>CFBundleShortVersionString</key>' "$packaged_plist" | grep -Fq '<string>1.1.27</string>' || \
    fail "packaged Cydia.app public version is not 1.1.27"
grep -A1 '<key>CFBundleVersion</key>' "$packaged_plist" | grep -Fq '<string>1.1.27</string>' || \
    fail "packaged Cydia.app does not select final bundle version 1.1.27"
...
echo "[ok] packaged Cydia.app is public version 1.1.27, final bundle version 1.1.27, with automatic Light/Dark appearance"
