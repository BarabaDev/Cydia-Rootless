 ...
 *   the build variant's root-owned Cydia application reference
 */
...
static bool TrollStoreBridgeIsSecure() {
    struct stat directory;
    struct stat link;

    if (lstat("/var/jb/.CydiaBridge", &directory) != 0 || !S_ISDIR(directory.st_mode)) {
        RootlessDiag("SECURITY", "level=ERROR bridge validation failed reason=missing-directory errno=%d", errno);
        return false;
    }
    if (directory.st_uid != 0 || (directory.st_mode & (S_IWGRP | S_IWOTH)) != 0) {
        RootlessDiag("SECURITY", "level=ERROR bridge validation failed reason=insecure-directory uid=%u mode=%o",
            directory.st_uid, directory.st_mode & 07777);
        return false;
    }
    if (lstat(CYDIA_APPLICATION_PATH, &link) != 0 || !S_ISLNK(link.st_mode)) {
        RootlessDiag("SECURITY", "level=ERROR bridge validation failed reason=app-reference-not-symlink errno=%d", errno);
        return false;
    }
    if (link.st_uid != 0) {
        RootlessDiag("SECURITY", "level=ERROR bridge validation failed reason=app-reference-not-root-owned uid=%u", link.st_uid);
        return false;
    }

    struct stat binary;
    if (stat(CYDIA_APPLICATION_BINARY, &binary) != 0 || !S_ISREG(binary.st_mode)) {
        RootlessDiag("SECURITY", "level=ERROR bridge validation failed reason=binary-unavailable errno=%d", errno);
        return false;
    }
    return true;
}
...
static bool CallerIsCydia() {
...
#if defined(CYDIA_TROLLSTORE_R2)
    if (!TrollStoreBridgeIsSecure()) {
        if (libproc != NULL)
            dlclose(libproc);
        fprintf(stderr, "cydo: TrollStore bridge is unavailable or insecure; refusing privileged request\n");
        return false;
    }
#endif
    ...
    for (unsigned depth = 0; pid > 1 && depth < 6; ++depth) {
        ...
        if (length > 0 && SameFile(path, CYDIA_APPLICATION_BINARY)) {
#if defined(CYDIA_TROLLSTORE_R2)
            struct stat identity;
            if (stat(CYDIA_APPLICATION_BINARY, &identity) == 0)
                RootlessDiag("SECURITY", "caller validation status=accepted variant=%s depth=%u device=%llu inode=%llu",
                    CYDIA_RUNTIME_VARIANT, depth,
                    static_cast<unsigned long long>(identity.st_dev),
...
#if defined(CYDIA_TROLLSTORE_R2)
    RootlessDiag("SECURITY", "level=ERROR caller validation reason=inode-mismatch variant=%s reference=%s",
        CYDIA_RUNTIME_VARIANT, CYDIA_APPLICATION_BINARY);
    fprintf(stderr, "cydo: caller ancestry does not contain the trusted Cydia binary; refusing privileged request\n");
#else
...
/*
 * Cydia's in-process APT planner intentionally runs without the system lock so
 * the mobile UI can inspect package state.  The privileged dpkg child is the
 * commit boundary, however, and must serialize with Sileo, Zebra and command
 * line APT.  Acquire both standard Debian locks here and retain them across
 * exec.  A bounded wait converts a short overlap into a normal delayed start
 * instead of an immediate half-finished transaction.
 */
static int AcquireDpkgLock(const char *path, unsigned timeoutSeconds) {
    int fd = open(path, O_RDWR | O_CREAT | O_NOFOLLOW, 0640);
    if (fd < 0) {
        RootlessDiag("DPKG_LOCK", "level=ERROR phase=open path=%s errno=%d error=%s", path, errno, strerror(errno));
        return -1;
    }
    ...
    for (;;) {
        ...
        if (saved != EACCES && saved != EAGAIN) {
            RootlessDiag("DPKG_LOCK", "level=ERROR phase=acquire path=%s errno=%d error=%s", path, saved, strerror(saved));
            close(fd);
            errno = saved;
            return -1;
        }
        ...
        if ((unsigned) (now.tv_sec - started.tv_sec) >= timeoutSeconds) {
            RootlessDiag("DPKG_LOCK", "level=ERROR phase=timeout path=%s waitedSeconds=%u", path, timeoutSeconds);
            close(fd);
            errno = EAGAIN;
            return -1;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2 || argv[1] == NULL || argv[1][0] != '/') {
        int frontendLock = AcquireDpkgLock("/var/jb/var/lib/dpkg/lock-frontend", 45);
        if (frontendLock < 0) {
            fprintf(stderr, "cydo: another package manager is still running; timed out waiting for the dpkg frontend lock\n");
            return EX_TEMPFAIL;
        }
        int databaseLock = AcquireDpkgLock("/var/jb/var/lib/dpkg/lock", 45);
        if (databaseLock < 0) {
            close(frontendLock);
            fprintf(stderr, "cydo: another package manager is still running; timed out waiting for the dpkg database lock\n");
            return EX_TEMPFAIL;
        }
        // Intentionally keep both descriptors open across exec. POSIX record
        // locks are inherited by this same process image and released when the
        // dpkg child exits.
        RootlessDiag("DPKG_LOCK", "status=acquired frontendFd=%d databaseFd=%d waitTimeoutSeconds=45", frontendLock, databaseLock);
