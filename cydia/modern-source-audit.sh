...
echo "== Cydia 1.1.30 clean rootless source audit =="
...
grep -A1 '<key>CFBundleShortVersionString</key>' MobileCydia.app/Info.plist | grep -Fq '<string>1.1.30</string>' || fail "app public version is not 1.1.30"
grep -A1 '<key>CFBundleVersion</key>' MobileCydia.app/Info.plist | grep -Fq '<string>1.1.30</string>' || fail "app build number is not the final release version 1.1.30"
...
[[ "$(grep -Fc 'CFSTR("Cydia/1.1.30")' "$http_method")" -eq 1 ]] || \
    fail "embedded HTTPS method does not contain exactly one Cydia/1.1.30 User-Agent"
...
grep -Fq '[footer setText:@"Cydia 1.1.30"]' Cydia/ModernNativeViews.mm || fail "Home footer version is missing"
