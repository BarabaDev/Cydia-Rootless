...
echo "== Cydia 1.1.22 clean rootless source audit =="
...
grep -A1 '<key>CFBundleShortVersionString</key>' MobileCydia.app/Info.plist | grep -Fq '<string>1.1.22</string>' || fail "app public version is not 1.1.22"
grep -A1 '<key>CFBundleVersion</key>' MobileCydia.app/Info.plist | grep -Fq '<string>1.1.22</string>' || fail "app build number is not the final release version 1.1.22"
...
[[ "$localization_file_count" == 41 ]] || fail "expected 41 active translation tables, found $localization_file_count"
...
[[ "$(grep -Fc 'CFSTR("Cydia/1.1.22")' "$http_method")" -eq 1 ]] || \
    fail "embedded HTTPS method does not contain exactly one Cydia/1.1.22 User-Agent"
...
grep -Fq 'request != nil && !launchCacheOnly && CydiaPrivacyConsentIsAccepted()' Cydia/ModernNativeViews.mm || fail "banner artwork can start a network request before first-launch acceptance"
...
grep -Fq 'CYM3CachedFeaturedBanner' Cydia/ModernNativeViews.mm || fail "previously seen banner artwork is not restored synchronously at launch"
...
grep -Fq 'request != nil && !launchCacheOnly' Cydia/ModernNativeViews.mm || fail "cache-only launch banners still start network requests"
...
grep -Fq 'CydiaSourceSpectrum' CyteKit/ModernAppearance.mm || fail "Sources top refresh bar lost its fluorescent palette"
...
grep -Fq '[footer setText:@"Cydia 1.1.22"]' Cydia/ModernNativeViews.mm || fail "Home footer version is missing"
...
grep -Fq '[title setText:@"Search Packages"]' MobileCydia.mm || fail "native Search empty state is missing"
...
grep -Fq '[percent_ setText:error_ || cancelled_ ? CYLocalize(@"Stopped") : @"100%"]' Cydia/ModernNativeViews.mm || fail "failed Progress can still present a misleading 100 percent label"
...
grep -Fq 'activeActions == 1 ? primaryAction : @"Apply Changes"' MobileCydia.mm || fail "confirmation button is not operation-aware"
...
grep -Fq '@"%lu upgrade%@  •  %lu dependenc%@"' MobileCydia.mm || fail "upgrade confirmation does not explain direct upgrades and dependencies separately"
grep -Fq 'classification=sileo-style' MobileCydia.mm || fail "transaction classification is not diagnosable"
grep -Fq '[[self navigationItem] setTitle:@"Review Changes"]' MobileCydia.mm || fail "confirmation still uses the oversized generic title"
