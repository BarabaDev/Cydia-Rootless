...
static NSString *CYHex(NSData *data, bool reverse = false) {
    ...
    if (length != 0)
        [data getBytes:bytes.data()];

    std::vector<char> string(length * 2 + 1, '\0');
    for (size_t i(0); i != length; ++i)
        snprintf(string.data() + i * 2, 3, "%.2x", bytes[reverse ? length - i - 1 : i]);

    return [NSString stringWithUTF8String:string.data()];
}
...
- (NSString *) getKernelString:(NSString *)name {
    ...
    static const size_t MaxKernelStringSize(1024 * 1024);
    if (size > MaxKernelStringSize)
        return (id) [NSNull null];
    std::vector<char> value(size + 1, '\0');
    if (sysctlbyname(string, value.data(), &size, NULL, 0) == -1)
        return (id) [NSNull null];
...
- (NSString *) stringWithFormat:(NSString *)format arguments:(WebScriptObject *)arguments {
    ...
    return [NSString stringWithFormat:format :count :values.data()];
}
