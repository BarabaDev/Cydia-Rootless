...
- (void) dealloc {
    if (kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iPhoneOS_3_0) {
        UIWebViewInternal **internal = MSHookIvarPointer<UIWebViewInternal *>(self, "_internal");
        if (internal != NULL && *internal != nil) {
            UIWebViewWebViewDelegate **webViewDelegate = MSHookIvarPointer<UIWebViewWebViewDelegate *>(*internal, "webViewDelegate");
            if (webViewDelegate != NULL && *webViewDelegate != nil)
...
static void $UIWebViewWebViewDelegate$webView$addMessageToConsole$(UIWebViewWebViewDelegate *self, SEL sel, WebView *view, NSDictionary *message) {
    UIWebView **slot(MSHookIvarPointer<UIWebView *>(self, "uiWebView"));
    UIWebView *uiWebView(slot == NULL ? nil : *slot);
...
static void $UIWebViewWebViewDelegate$_clearUIWebView(UIWebViewWebViewDelegate *self, SEL sel) {
    if (UIWebView **slot = MSHookIvarPointer<UIWebView *>(self, "uiWebView"))
        *slot = nil;
}
