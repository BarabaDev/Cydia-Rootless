...
#define CYMailTrace(...) CYRootlessDiag(@"SUPPORT", __VA_ARGS__)

static void $MFMailComposeViewController$setMailToURL$(MFMailComposeViewController *self, SEL _cmd, NSURL *url) {
    CYMailTrace(@"setMailToURL self=%@ url=%@", self, CYRootlessDiagnosticsURL(url));
...
__attribute__((__constructor__)) static void MFMailComposeViewController_CyteMailToURL() {
    void *handle(dlopen("/System/Library/Frameworks/MessageUI.framework/MessageUI", RTLD_GLOBAL | RTLD_LAZY));
    Class MFMailComposeViewController(objc_getClass("MFMailComposeViewController"));
    BOOL added(NO);
    if (MFMailComposeViewController != Nil)
        added = class_addMethod(MFMailComposeViewController, @selector(setMailToURL:), (IMP) $MFMailComposeViewController$setMailToURL$, "v12@0:4@8");
    CYMailTrace(@"constructor handle=%p class=%@ class_addMethod=%d responds=%d", handle, MFMailComposeViewController, (int) added, MFMailComposeViewController != Nil ? (int) [MFMailComposeViewController instancesRespondToSelector:@selector(setMailToURL:)] : -1);
}
