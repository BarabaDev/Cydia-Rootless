...
#define CYSupportTrace(...) CYRootlessDiag(@"SUPPORT", __VA_ARGS__)
...
// UIWebView is backed by WebKitLegacy and its Page teardown is no longer
// reliable on iOS 17.  In particular, releasing a WebView while WebThread is
// retiring a timer can crash inside TimerBase/Page::~Page (the two device
// reports from 2026-08-30/31 show this exact stack).  Cydia is a long-running
// package manager with only a small number of web-backed controllers, so on
// iOS 17+ detach and park retired views for the lifetime of the process.  This
// avoids invoking the broken WebKitLegacy destructor; the operating system
// reclaims the pages when Cydia exits.
static _H<NSMutableArray> RetiredLegacyWebViews_;

static BOOL CYRequiresLegacyWebViewRetirement(void) {
    NSOperatingSystemVersion version([[NSProcessInfo processInfo] operatingSystemVersion]);
    return version.majorVersion >= 17;
}

static void CYRetireLegacyWebView(CyteWebView *view) {
    if (view == nil)
        return;
    ...
    if (CYRequiresLegacyWebViewRetirement()) {
        if (RetiredLegacyWebViews_ == nil)
            RetiredLegacyWebViews_ = [NSMutableArray arrayWithCapacity:8];
        [RetiredLegacyWebViews_ addObject:view];
        CYRootlessDiag(@"WEBVIEW", @"action=retired reason=ios17-webkitlegacy-timer-teardown retained=%lu",
            (unsigned long) [RetiredLegacyWebViews_ count]);
    }
}
...
+ (void) _initialize {
    ...
    CYSupportTrace(@"initialize MessageUI class=%@ canSendMail=%d", $MFMailComposeViewController, $MFMailComposeViewController != nil ? (int) [$MFMailComposeViewController canSendMail] : -1);
...
- (void) dealloc {
    ...
    // UIViewController's private teardown otherwise releases UIWebView
    // directly, bypassing -releaseSubviews.  Route it through our iOS 17
    // retirement path before superclass destruction starts.
    if ([self isViewLoaded])
        [self setView:nil];
    else if (webview_ != nil)
...
- (void) mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    CYSupportTrace(@"mailComposeController finish result=%ld error=%@", (long) result, error);
...
- (void) _openMailToURL:(NSURL *)url {
    BOOL available($MFMailComposeViewController != nil);
    BOOL canSend(available ? [$MFMailComposeViewController canSendMail] : NO);
    CYSupportTrace(@"_openMailToURL url=%@ class=%@ available=%d canSendMail=%d", CYRootlessDiagnosticsURL(url), $MFMailComposeViewController, (int) available, (int) canSend);

    if (available && canSend) {
        MFMailComposeViewController *controller([[[$MFMailComposeViewController alloc] init] autorelease]);
        CYSupportTrace(@"composer allocated=%@ selector(setMailToURL:)=%d", controller, (int) [controller respondsToSelector:@selector(setMailToURL:)]);
        [controller setMailComposeDelegate:self];

        @try {
            [controller setMailToURL:url];
            CYSupportTrace(@"composer setMailToURL completed url=%@", CYRootlessDiagnosticsURL(url));
        } @catch (NSException *exception) {
            CYSupportTrace(@"setMailToURL exception=%@ reason=%@", [exception name], [exception reason]);
            return;
        }

        // Diagnostics proved MessageUI construction succeeds and the
        // trace stopped at the optional attachment setup. Attachment failures
        // must never prevent the Support composer from appearing.
        @try {
            CYSupportTrace(@"mail attachment setup begin");
            [self _setupMail:controller];
            CYSupportTrace(@"mail attachment setup complete");
        } @catch (NSException *exception) {
            CYSupportTrace(@"mail attachment setup exception=%@ reason=%@; continuing", [exception name], [exception reason]);
        }
        ...
        CYSupportTrace(@"presenting mail composer presenter=%@ inWindow=%d", presenter, (int) ([[presenter view] window] != nil));
        @try {
            if ([presenter respondsToSelector:@selector(presentViewController:animated:completion:)])
                [presenter presentViewController:controller animated:YES completion:nil];
            else
                [presenter presentModalViewController:controller animated:YES];
            CYSupportTrace(@"mail composer presentation requested");
        } @catch (NSException *exception) {
            CYSupportTrace(@"mail composer presentation exception=%@ reason=%@", [exception name], [exception reason]);
        }
        return;
    }

    UIApplication *app([UIApplication sharedApplication]);
    CYSupportTrace(@"MessageUI unavailable/cannot send; canOpenURL=%d", (int) [app canOpenURL:url]);
    if ([app respondsToSelector:@selector(openURL:options:completionHandler:)]) {
        CYSupportTrace(@"fallback modern UIApplication openURL=%@", CYRootlessDiagnosticsURL(url));
        [app openURL:url options:[NSDictionary dictionary] completionHandler:nil];
    } else if ([app respondsToSelector:@selector(openURL:asPanel:)]) {
        CYSupportTrace(@"fallback legacy openURL:asPanel=%@", CYRootlessDiagnosticsURL(url));
        [app openURL:url asPanel:YES];
    } else {
        CYSupportTrace(@"fallback legacy openURL=%@", CYRootlessDiagnosticsURL(url));
        [app openURL:url];
    }
}
...
- (void) webView:(WebView *)view decidePolicyForNewWindowAction:(NSDictionary *)action request:(NSURLRequest *)request newFrameName:(NSString *)name decisionListener:(id<WebPolicyDecisionListener>)listener {
    ...
    // Support/Author e-mail rows are emitted as target="_open".
    // On modern iOS that meant the old _open branch consumed mailto: before
    // Cydia's original MessageUI composer ever saw it.  Route mailto: first,
    // regardless of the target name, and preserve the historical routing for
    // every non-mail URL.
    NSString *scheme([[url scheme] lowercaseString]);
    CYSupportTrace(@"newWindow url=%@ scheme=%@ target=%@", CYRootlessDiagnosticsURL(url), scheme, name);
    if ([scheme isEqualToString:@"mailto"]) {
        CYSupportTrace(@"route=mailto -> _openMailToURL");
        [self _openMailToURL:url];
    } else if ([name isEqualToString:@"_open"]) {
        CYSupportTrace(@"route=_open -> delegate openURL");
        [self.delegate openURL:url];
    } else {
        CYSupportTrace(@"route=pushRequest popup=%d", (int) [name isEqualToString:@"_popup"]);
...
- (id) initWithWidth:(float)width ofClass:(Class)_class {
    if ((self = [super init]) != nil) {
        ...
        [loadingview addSubview:indicator_];

        loadingitem_ = [[[UIBarButtonItem alloc] initWithCustomView:loadingview] autorelease];
...
- (void) loadView {
    ...
    if ([[UIColor groupTableViewBackgroundColor] isEqual:[UIColor clearColor]]) {
        UITableView *table([[[UITableView alloc] initWithFrame:[webview_ bounds] style:CYModernGroupedTableStyle()] autorelease]);
        CYModernizeTableView(table);
...
- (void) callFunction:(WebScriptObject *)function {
    ...
    id target([CyteWebViewController class]);
...
- (void) setNavigationBarStyle:(NSString *)name {
    ...
    if ([name isEqualToString:@"Black"])
        CYModernizeDarkNavigationController(navigation);
    else
